...
describe("ajx.patch function", () => {
    ...
    describe("ajx.patch for '/204' with string 'hello' and no option:", () => {
        let responseBody = null
        let requests = null

        before(co.wrap(function* () {
            responseBody = yield ajx.patch(server.url("/204"), "hello")
        ...
        describe("The server", () => {
            it("should receive a PATCH request", () => {
    ...
    describe("ajx.patch for '/204' with object '{hello: \"world\"}' and no option:", () => {
        let responseBody = null
        let requests = null

        before(co.wrap(function* () {
            responseBody = yield ajx.patch(server.url("/204"), {hello: "world"})
            requests = server.result()
        }))
    ...
    describe("ajx.patch for '/204' with a FormData object and no option:", () => {
        let responseBody = null
        let requests = null
        ...
        before(co.wrap(function* () {
            ...
            responseBody = yield ajx.patch(server.url("/204"), formData)
            requests = server.result()
        }))
    ...
    describe("ajx.patch for '/204' with a HTMLFormElement object and no option:", () => {
        let responseBody = null
        let requests = null
        ...
        before(co.wrap(function* () {
            ...
            responseBody = yield ajx.patch(server.url("/204"), form)
            requests = server.result()
        }))
